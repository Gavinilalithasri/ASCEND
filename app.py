...
def get_news():
    ...
    page_size = request.args.get('page_size', 5)  # Default to 5 articles
...
# Example route to render a template (optional)
@app.route('/news.html')
def news_page():
    return render_template('news.html')
#-----------------------------------------------------------------------------JobPrediction-----------------------------------------------------------------
# Add this near your other genai.configure calls (you can consolidate API keys if needed)
genai.configure(api_key="AIzaSyAWwbp45qtLqELSaAg1yB3uiTaQITVF-aE")  # From your script
predict_model = genai.GenerativeModel("gemini-1.5-flash")

# New /predict route for career path prediction
...
def predict():
    try:
        ...
        if not interests or not skills:
            return jsonify({"error": "Please provide both interests and skills"}), 400

        # Format the prompt (adapted from your script)
        system_content = (
            "You are an expert career advisor specializing in job role prediction. "
            "Based on the provided list of interests and skills, suggest the top 5-8 most relevant job roles. "
            "Consider how the interests and skills align with various career paths and responsibilities. "
            "Provide only the job role names in a comma-separated list."
        )
        user_content = f"Interests: {interests}\nSkills: {skills}"
        prompt = f"System: {system_content}\n\nUser Input: {user_content}\n\nPredicted Job Roles:"

        # Call Gemini API
        response = predict_model.generate_content(prompt)
        predicted_roles_str = response.text.strip()
        predicted_roles = [role.strip() for role in predicted_roles_str.split(',')]

        return jsonify({"roles": predicted_roles})
    except Exception as e:
        return jsonify({"error": f"Error predicting roles: {str(e)}"}), 500
#------------------------------------------------------------------------------Routes----------------------------------------------------------------
@app.route('/')
def index():
...
@app.route('/hugg')
def hugg():
    return render_template('hugg.html')
...
@app.route('/joinget.html')
def joinget():
  return render_template('joinget.html')
...
def chatbotw():
    ...
    prompt = f"""
    You are a chatbot for ASCEND. Use the following website details to assist users:
    {pdf_text}
...
# Configure Google Gemini API
GEMINI_API_KEY = "AIzaSyAs3Im68Uu9kj5vLWt6K6A7vFJIayzNPL4"  
...
def get_gemini_response(user_input):
    try:
        ...
        # Define a natural and conversational prompt
       
        prompt = (
            "You are a highly intelligent and friendly career mentor chatbot. "
            "Respond naturally like ChatGPT, keeping answers clear, structured, and engaging. "
            "Use HTML formatting for clarity. Maintain a professional but conversational tone.\n\n"
            f"User Query:\n{user_input}\n\n"
            "Format your response in structured HTML with appropriate <b>bold headings</b> and bullet points (<ul><li>...</li></ul>) where necessary. "
            "Ensure clarity, conciseness, and relevance. Avoid excessive repetition. Provide step-by-step guidance where applicable."
        )
